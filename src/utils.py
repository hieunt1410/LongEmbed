...
token_per_word: float = (
    4 / 3
)  # using empirical evidence that 1 token is about 3/4 of a word
...
def get_chunked_docs(
    args: argparse.Namespace, doc_list: List[str]
    ...
    logger.info(
        f"start chunking corpus, chunk_size: {chunk_size}, chunk_overlap: {chunk_overlap}, max_token_num: {max_token_num}"
    )
...
def get_args(input_args: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pos_mode", type=str, default="original", help="original / recurrent / group"
    )
    parser.add_argument("--pool_type", type=str, default="avg", help="pool type")
    parser.add_argument(
        "--prefix_type", type=str, default="query_or_passage", help="prefix type"
    )
    ...
    logger.info(
        "Args: {}".format(json.dumps(args.__dict__, ensure_ascii=False, indent=4))
    )
    assert args.pool_type in ["cls", "avg", "last", "weightedavg"], (
        "pool_type should be cls / avg / last"
    )
