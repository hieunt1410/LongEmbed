...
from selfextend_flash_attn import (
    self_extend_flash_forward,
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    ...
    q_embed = (
        (q * cos[:, :, -q.shape[2] :]) + (rotate_half(q) * sin[:, :, -q.shape[2] :])
        if q is not None
        else None
    )
...
def apply_grouped_rotary_pos_emb(
    q, k, cos, sin, position_ids, g_size_1=1, g_size_2=4096
):
    # The first two dimensions of cos and sin are always 1, so we can `squeeze` them.
...
def apply_grouped_rotary_pos_emb_upper(
    q, k, cos, sin, position_ids, g_size_1=1, g_size_2=4096
):
    # The first two dimensions of cos and sin are always 1, so we can `squeeze` them.
    ...
    **kwargs,
    ...
    if scale_base > 0:
        scaled_query = query_states * (
            (position_ids + 1)[:, None, :, None].log() / np.log(scale_base)
        ).clip(1).to(query_states.dtype)  # log scale
    ...
    key_position = (
        torch.arange(kv_seq_len, dtype=position_ids.dtype)
    ...
    neighbor_query_states, _ = apply_rotary_pos_emb(
        scaled_query, None, cos, sin, query_position
    )
    _, neighbor_key_states = apply_rotary_pos_emb(
        None, key_states, cos, sin, key_position
    )
    _re_group_size_2 = (
    ...
    group_query_states, _ = apply_grouped_rotary_pos_emb(
        scaled_query,
        None,
        cos,
        sin,
        query_position,
        g_size_1=group_size_1,
        g_size_2=_re_group_size_2,
    )
    _, group_key_states = apply_grouped_rotary_pos_emb(
        None,
        key_states,
        cos,
        sin,
    ...
    **kwargs,
    ...
    if q_len == 1:
        ...
        decode_key_position = torch.cat(
            [
                group_key_position[:, :-group_size_2],
                neighbor_key_position[:, -group_size_2:],
            ],
            dim=1,
        )
        ...
        decode_query_states = scaled_query.transpose(
            1, 2
        ).contiguous()  # position 0: cos 0 = 1, sin 0 = 0
        _, decode_key_states = apply_rotary_pos_emb(
            None, key_states, cos, -sin, decode_key_position
        )

        decode_key_states = (
            repeat_kv(decode_key_states, self.num_key_value_groups)
            .transpose(1, 2)
            .contiguous()
        )
        decode_value_states = (
        ...
        neighbor_key_states = (
            repeat_kv(neighbor_key_states, self.num_key_value_groups)
            .transpose(1, 2)
            .contiguous()
        )
        group_query_states = group_query_states.transpose(1, 2).contiguous()
        group_key_states = (
            repeat_kv(group_key_states, self.num_key_value_groups)
            .transpose(1, 2)
            .contiguous()
        )
        value_states = (
        ...
        attn_output = self_extend_flash_forward(
            self,
            query_position,
            group_size_2,
...
def e5rope_self_extend_forward(
    ...
    cos, sin = self.rotary_emb(value_layer, seq_len=kv_seq_len)
    neighbor_query_states, neighbor_key_states = apply_rotary_pos_emb(
    ...
    if q_len < 512:
        ...
        attn_output = xops.memory_efficient_attention(
            neighbor_query_states.transpose(1, 2),
            neighbor_key_states.transpose(1, 2),
    ...
    group_query_states_upper, group_key_states_upper = (
        apply_grouped_rotary_pos_emb_upper(
            query_states,
            key_states,
            cos,
            sin,
    ...
    if past_key_value is not None:
        ...
        group_key_states = torch.cat(
            [past_key_value[1], group_key_states], dim=2
        )  # cache group_key_states
        value_states = torch.cat([past_key_value[2], value_states], dim=2)

    neighbor_attn_weights = torch.matmul(
        neighbor_query_states, neighbor_key_states.transpose(2, 3)
    ) / math.sqrt(self.head_dim)
    group_attn_weights_lower = torch.matmul(
        group_query_states, group_key_states.transpose(2, 3)
    ) / math.sqrt(self.head_dim)
    group_attn_weights_upper = torch.matmul(
        group_query_states_upper, group_key_states_upper.transpose(2, 3)
    ) / math.sqrt(self.head_dim)

    # merge group_attn_weights and group_attn_weights_upper
    group_attn_weights = torch.tril(group_attn_weights_lower) + torch.triu(
        group_attn_weights_upper, diagonal=1
    )
    ...
    if q_len == 1:
        ...
        if group_size_2 == 0:
            ...
            group_attention_mask_lower = torch.tril(
                torch.ones(
                    (q_len - group_size_2, kv_seq_len - group_size_2),
                    device=group_attn_weights.device,
                )
            ...
            neighbor_attention_mask[group_size_2:, :-group_size_2] -= (
                group_attention_mask_lower
            )

            group_attention_mask_upper = torch.triu(
                torch.ones(
                    (q_len - group_size_2, kv_seq_len - group_size_2),
                    device=group_attn_weights.device,
                )
    ...
    merged_attn_weights = nn.functional.softmax(
        merged_attn_weights, dim=-1, dtype=torch.float32
    ...
    outputs = (
        (context_layer, merged_attn_weights) if output_attentions else (context_layer,)
    )
